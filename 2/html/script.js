...
var consoleDescriptionDismissed = false;
var ifr;
var ifrw;
var console;
var text;
...
// Makes tab function properly (makes it add spaces instead of tabs)
...
// Adds Ctrl-G shortcut, to fix indentation
CodeMirror.keyMap.default[(navigator.platform.match("Mac") ? "Cmd" : "Ctrl") + "-G"] = "indentAuto";

// Automatically show autocomplete menu
...
// Show different CodeMirror hints for CSS
var orig = CodeMirror.hint.css;

CodeMirror.hint.css = function(cm) {
    var inner = orig(cm) || {from: cm.getCursor(), to: cm.getCursor(), list: []};
    if (inner.list == "") {
        inner.list.push("Emmet abbreviation");
    }
    return inner;
};
...
// Set size of editor to take all available space
...
function showOutput(cooldown) {
    if (runButtonActive) {
        if (navigator.onLine) {
            ...
            document.getElementById("output-container").innerHTML = "";
            document.getElementById("output-container").appendChild(ifr);
            document.getElementById("output-container").appendChild(console);
            var consoleProperScrollingElement = document.createElement("div");
            consoleProperScrollingElement.setAttribute("class", "info");
            consoleProperScrollingElement.style.visibility = "hidden";
            consoleProperScrollingElement.innerHTML = "d";
            console.appendChild(consoleProperScrollingElement);
            ifrw = (ifr.contentWindow) ? ifr.contentWindow: (ifr.contentDocument.document) ? ifr.contentDocument.document: ifr.contentDocument;
            var oldLog = ifrw.console.log;
            ifrw.console.log = function(...args) {
                oldLog.apply(console, args);
                var consoleInfoLine = document.createElement("div");
                consoleInfoLine.setAttribute("class", "info");
                consoleInfoLine.innerText = args.join(" ");
                document.getElementById("console").appendChild(consoleInfoLine);
                consoleProperScrollingElement.remove();
                document.getElementById("console").appendChild(consoleProperScrollingElement);
            }
            var oldInfo = ifrw.console.info;
            ifrw.console.info = function(...args) {
            ...
            var oldWarn = ifrw.console.warn;
            ifrw.console.warn = function(...args) {
                oldWarn.apply(console, args);
                var consoleWarningLine = document.createElement("div");
                consoleWarningLine.setAttribute("class", "warning");
                consoleWarningLine.innerText = args.join(" ");
                document.getElementById("console").appendChild(consoleWarningLine);
                consoleProperScrollingElement.remove();
                document.getElementById("console").appendChild(consoleProperScrollingElement);
            }
            var oldError = ifrw.console.error;
            ifrw.console.error = function(...args) {
                oldError.apply(console, args);
                var consoleErrorLine = document.createElement("div");
                consoleErrorLine.setAttribute("class", "error");
                consoleErrorLine.innerText = args.join(" ");
                document.getElementById("console").appendChild(consoleErrorLine);
                consoleProperScrollingElement.remove();
                document.getElementById("console").appendChild(consoleProperScrollingElement);
            }
            ifrw.document.open();
            ...
            consoleInput.setAttribute("class", "console-input");
            consoleInput.innerHTML = "<span style='user-select: none; font-family: Consolas; font-size: 0.97rem;'>&gt;</span> <input type='text' spellcheck='false' autocomplete='off' style='background-color: transparent; border: none; outline: none; width: calc(100% - 1.06rem); padding: 0;' id='console-text-input' />";
            document.getElementById("console").appendChild(consoleInput);
            document.getElementById("console-text-input").onkeydown = function(e) {
                if (e.key == "Enter") {
                    var consoleInputToScript = ifrw.document.createElement("script");
                    consoleInputToScript.text = document.getElementById("console-text-input").value;
                    ifrw.document.body.appendChild(consoleInputToScript);
                    document.getElementById("console-text-input").value = "";
                    consoleProperScrollingElement.remove();
                    document.getElementById("console").appendChild(consoleProperScrollingElement);
                }
            }
...
function getFileSize() {
    var codeLength = editor.getValue().length;
    ...
    Array.prototype.slice.call(forms).forEach(function(form) {
        form.addEventListener('submit', function(event) {
            if (!form.checkValidity()) {
                ...
                downloadCode(document.getElementById('custom-filename').value + '.' + document.getElementById('custom-file-extension').value, editor.getValue())
            }
