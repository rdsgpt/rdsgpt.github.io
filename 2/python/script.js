...
function showOutput(cooldown) {
    if (runButtonActive) {
        if (navigator.onLine) {
            ...
            ifrw = (ifr.contentWindow) ? ifr.contentWindow: (ifr.contentDocument.document) ? ifr.contentDocument.document: ifr.contentDocument;
            ifrw.document.open();
            ifrw.document.writeln('<!DOCTYPE html><html><head><link rel="stylesheet" href="https://pyscript.net/alpha/pyscript.css" /><script defer src="https://pyscript.net/alpha/pyscript.js"></script><style>@import url("https://fonts.googleapis.com/css?family=IBM Plex Mono"); * {cursor: default !important;} *::selection {background: white !important; color: black !important;}</style></head><body style=\'padding: 8px; background-color: black; color: white; font-size: 14px; font-family: "IBM Plex Mono", monospace;\' oncontextmenu="event.preventDefault();"><py-script>' + text.replace(RegExp("</py-script>", "gi"), "") + "</py-script></body></html>");
            ifrw.document.close();
