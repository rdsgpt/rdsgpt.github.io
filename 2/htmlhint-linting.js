(function (global, factory) {
	...
	doctypeFirst.default = {
	    id: 'doctype-first',
	    description: 'A !DOCTYPE must be declared first.',
	    init(parser, reporter) {
	        const allEvent = (event) => {
	            ...
	            if ((event.type !== 'comment' && event.long === false) ||
	                ...
	                reporter.error('A !DOCTYPE must be declared first.', event.line, event.col, this, event.raw);
	            }
	...
	spaceTabMixedDisabled.default = {
	    ...
	    description: 'Avoid mixing tabs and spaces for indentation.',
	    init(parser, reporter, options) {
	                ...
	                if (indentMode === 'space') {
	                    if (spaceLengthRequire) {
	                        if (/^ +$/.test(whiteSpace) === false ||
	                            ...
	                            reporter.warn(`Please use spaces for indentation and keep a length of ${spaceLengthRequire} spaces.`, fixedPos.line, 1, this, event.raw);
	                        }
	                    }
	                    else {
	                        if (/^ +$/.test(whiteSpace) === false) {
	                            reporter.warn('Please use spaces for indentation.', fixedPos.line, 1, this, event.raw);
	                        }
	                    }
	                }
	                else if (indentMode === 'tab' && /^\t+$/.test(whiteSpace) === false) {
	                    reporter.warn('Please use tabs for indentation.', fixedPos.line, 1, this, event.raw);
	                }
	                else if (/ +\t|\t+ /.test(whiteSpace) === true) {
	                    reporter.warn('Avoid mixing tabs and spaces for indentation.', fixedPos.line, 1, this, event.raw);
	                }
	            }
	        });
	    },
	...
	tagPair.default = {
	    id: 'tag-pair',
	    description: 'All non-empty tags must be paired.',
	    init(parser, reporter) {
	        const stack = [];
	        ...
	        parser.addListener('tagend', (event) => {
	            ...
	            if (pos >= 0) {
	                ...
	                if (arrTags.length > 0) {
	                    ...
	                    reporter.error(`All non-empty tags must be paired, missing (${arrTags.join('')}), start tag match failed (${lastEvent.raw}) on line ${lastEvent.line}.`, event.line, event.col, this, event.raw);
	                }
	                stack.length = pos;
	            }
	            else {
	                reporter.error(`All non-empty tags must be paired, no start tag (${event.raw})`, event.line, event.col, this, event.raw);
	            }
	        });
	        parser.addListener('end', (event) => {
	            ...
	            if (arrTags.length > 0) {
	                const lastEvent = stack[stack.length - 1];
	                reporter.error(`All non-empty tags must be paired, missing (${arrTags.join('')}), open tag match failed (${lastEvent.raw}) on line ${lastEvent.line}.`, event.line, event.col, this, '');
	            }
	        });
	    },
	};
	...
	tagSelfClose.default = {
	    id: 'tag-self-close',
	    description: 'All empty tags must be self closed.',
	    init(parser, reporter) {
	        ...
	        const onTagEnd = (event) => {
	            ...
	            if (hasTitle && tagName === 'title') {
	                ...
	                if (lastEvent.type !== 'text' ||
	                    ...
	                    reporter.error('The <title> of a document must not be empty.', event.line, event.col, this, event.raw);
	                }
	            }
	            else if (tagName === 'head') {
	                if (hasTitle === false) {
	                    reporter.error('A <title> must be present in the <head> tag.', event.line, event.col, this, event.raw);
	                }
	                parser.removeListener('tagstart', onTagStart);
	                parser.removeListener('tagend', onTagEnd);
	            }
	                            ...
	                            if (attrs.some((attr) => attr.name === realID)) {
	                                attrs.forEach((attr) => {
	                                    if (attr.name === realID &&
	                                        ...
	                                        reporter.error(`The <${tagName}> tag must have attribute '${realID}' with one value of '${values.join("' or '")}'.`, event.line, col, this, event.raw);
	                                    }
	                                });
	                            }
	                            else {
	                                reporter.error(`The <${tagName}> tag must have attribute '${realID}'.`, event.line, col, this, event.raw);
	                            }
	                        }
	                        else if (!attrs.some((attr) => id.split('|').indexOf(attr.name) !== -1)) {
	                            reporter.error(`The <${tagName}> tag must have attribute '${id}'.`, event.line, col, this, event.raw);
	                        }
	                    });
	                }
	                        ...
	                        if (Array.isArray(id)) {
	                            ...
	                            if (attrs.some((attr) => attr.name === realID)) {
	                                attrs.forEach((attr) => {
	                                    if (attr.name === realID &&
	                                        values.indexOf(attr.value) === -1) {
	                                        reporter.error(`The <${tagName}> tag must have optional attribute '${realID}' with one value of '${values.join("' or '")}'.`, event.line, col, this, event.raw);
	                                    }
	                                });
	                            }
	                        }
	                ...
	                if (Array.isArray(currentTagType.redundantAttrs)) {
	                    ...
	                    redundantAttrs.forEach((attrName) => {
	                        if (attrs.some((attr) => attr.name === attrName)) {
	                            reporter.error(`The attribute '${attrName}' is redundant for <${tagName}> and should be omitted.`, event.line, col, this, event.raw);
	                        }
	                    });
	                }
	            }
