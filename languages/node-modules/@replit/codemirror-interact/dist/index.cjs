...
const mark = view.Decoration.mark({ class: 'cm-interact' });
const setInteract = state.StateEffect.define();
const interactTheme = view.EditorView.theme({
    '.cm-interact': {
 ...
 * ```
 */
const interactRule = state.Facet.define();
const interactModKey = state.Facet.define({
    combine: (values) => values[values.length - 1],
});
const interactViewPlugin = view.ViewPlugin.define((view$1) => ({
    dragging: null,
    hovering: null,
    mouseX: 0,
    mouseY: 0,
    deco: view.Decoration.none,
    // Get current match under cursor from all rules
    getMatch() {
        const rules = view$1.state.facet(interactRule);
        const pos = view$1.posAtCoords({ x: this.mouseX, y: this.mouseY });
        if (!pos)
            return null;
    ...
    isModKeyDown(e) {
        const modkey = view$1.state.facet(interactModKey);
        switch (modkey) {
        ...
        for (const tr of update.transactions) {
            for (const e of tr.effects) {
                if (e.is(setInteract)) {
                    ...
                    this.deco = view.Decoration.set(decos);
                }
            }
        }
    },
...
exports.interactModKey = interactModKey;
exports.interactRule = interactRule;
